package inu.amigo.orderIt.global.jwt.filter;

import inu.amigo.orderIt.domain.user.repository.UserRepository;
import inu.amigo.orderIt.global.jwt.util.PasswordUtil;
...
public class JwtAuthenticationProcessingFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (request.getRequestURI().equals(NO_CHECK_URL)) {
            log.info("[doFilterInternal] Login URL 예외처리");
        ...
        if (refreshToken != null) {
            log.info("[refreshToken] is existed");
    ...
    public void checkRefreshTokenAndReIssueAccessToken(HttpServletResponse response, String refreshToken) {
        ...
        log.info("[checkRefreshTokenAndReIssueAccessToken] 종료");
    }
     ...
     * 유효한 토큰이면, 액세스 토큰에서 extractUsername로 Username을 추출한 후 findByUsername()로 해당 이메일을 사용하는 유저 객체 반환
    ...
    public void saveAuthentication(User myUser) {
        log.info("[saveAuthentication] 호출 user : {}", myUser.toString());
